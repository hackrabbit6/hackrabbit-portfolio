...
> **状态：P0–P7 已完成（PR #1）。本文档描述的是「个人作品站」那一版的视觉整改。**
> 2026-08-12 站点定位改为「个人主页」（见 `AGENTS.md`），本文档中「明确不做」里
> 关于新增路由的那几条已由本人解除。文档中的字号刻度、间距节奏、Cover 渲染管线、
> 移动端与可访问性结论继续有效，不随定位改变而作废。
...
Cover 由 `tools/covers/` 下的同一套 HTML + CSS 渲染成 WebP，共用 `cover.css`
...
| # | 出图尺寸（CSS） | 倍率 | 视觉位 |
|---|---|---|---|
| 01 | 684 × 520 | 2x | `.work-featured`（桌面） |
| 02 / 03 | 322 × 360 | 4x | `.work-pair`（窄屏并成单列后会被拉到约 640，故需 4x） |
| 04 | 684 × 480 | 2x | `.work-wide`（桌面） |
| 01m / 04m | 340 × 454 / 464 | 4x | 窄屏专版，`<picture>` 在 767px 切换（见 P6） |
...
01 与 04 另有 340 宽的窄屏专版，用 `<picture>` 在 767px 切换（见 P6）。
...
Cover 是位图，字画在图里，不会随容器重新排版。窄屏下整张按比例缩小，
04 的终端正文（按 684 宽设计的 11px）在 390px 上只剩约 5.5px，认不出字。

只有 01 和 04 有这个问题 —— 它们按 684 出图，手机上被缩到 0.5×。
02/03 本来就是 322 宽，手机单列时接近 1:1，不需要专版。

补了 `01m-` / `04m-` 两张 340 宽的窄屏版，用 `<picture>` 的
`<source media="(max-width: 767px)">` 切换。**这个 767px 必须和 `Work.css`
里改 `aspect-ratio` 的断点完全一致** —— 换了图没换比例的话，
`object-fit: cover` 会直接把图裁掉。

04 窄屏版的字号是反算出来的：表格最宽一行 47 字符（Cluster 表头与分隔线），
Menlo 字宽 0.6001em，面板内宽 292px → 10px 是不把表格结构拆散的上限，
再大一档表头就折行。折行的只有两行 prose 和两条警告，真实窄终端也是这么折的。

顺带把六张 Cover 从 PNG 换成 WebP（q95，Chrome 直接编码，肉眼无差别）：
桌面一次访问 1109KB → 246KB，手机 1447KB → 375KB。
`<picture>` 只下命中的那一张，不会两版都下。
...
- ~~新建 `/about/`、`/now/`、`/lab/`、`/writing/` 或任何新路由~~
  （2026-08-12 解除：定位改为个人主页后允许扩展路由，条件见 `AGENTS.md`）
- 更换框架、包管理器、部署平台
